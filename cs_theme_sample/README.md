...
## Todo
- We need to figure out a way to copy over the `drupal.js` and `drupal.init.js` files from the current build's core, instead of cURLing them from Drupal's repo.
- Revisit ESLint rules, we may need to ignore some specific ES6 rules that are causing code to fail validation. 
